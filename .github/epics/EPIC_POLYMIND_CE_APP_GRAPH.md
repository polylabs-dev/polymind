...
Register PolyMind's 13 FL circuit modules into the platform App Graph, wire intra-graph REQUIRES edges and cross-graph BRIDGE_TO edges (PolyKit ESLM classify, PolyDocs document ingest), and configure the Cognitive Engine with 3 meaning domains, noise filter, and 3 SME panels.
...
- [ ] Implement `make_polymind_module` helper with PolyMind-common defaults
...
- [ ] Implement `polymind_app_graph_register` with 19+ REQUIRES edges
- [ ] Implement `polymind_register_bridge_edges` (PolyKit classify, PolyDocs parser)
...
- [ ] Implement `polymind_register_ce` orchestrator
- [ ] Add golden tests for domains, noise filter, panels, registration

### Integration

- [ ] Verify bilateral bridge confirmation with PolyKit eslm_classify
...
1. `polymind_app_graph_register` adds exactly 13 modules and >=19 edges
2. `polymind_register_bridge_edges` adds 2 BRIDGE_TO cross-graph edges
3. `polymind_register_ce` registers 3 domains, 6 noise rules, 3 SME panels
